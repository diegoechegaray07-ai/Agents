...
def resolver_propietario(nombre_establecimiento):
    clave = nombre_establecimiento.upper().replace(' ', '')
    return PROPIETARIOS.get(clave, nombre_establecimiento)
...
if __name__ == '__main__':
    ...
    propietario = resolver_propietario(sys.argv[2])
    salida = sys.argv[3]
    ...
    rango = desde if desde == hasta else f'{desde}_{hasta}'

    directorio = os.path.dirname(salida)
    prop_slug = propietario.replace(' ', '_')
    salida_con_fecha = os.path.join(directorio, f'Informe_{prop_slug}_{rango}.pdf')

    generar_informe(sys.argv[1], propietario, salida_con_fecha)
